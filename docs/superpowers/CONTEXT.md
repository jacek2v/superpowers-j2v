...
- **D-045** ✗ DO NOT build a flow in which a subagent waits for an event — a subagent ends the moment it stops calling tools, so only the main session can hold a `Monitor` (measured 2026-08-29: it armed the watch, returned after 15 s, and no event reached it afterwards) [2026-08-29](specs/2026-08-29-gated-green-implementer-design.md)
